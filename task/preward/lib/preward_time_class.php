...
// 		//威客的记录集合，更新
// 		$nomark_wk_list = db_factory::query(sprintf('select `mark_id` from %switkey_mark where model_code="%s" and mark_status=0 and mark_max_time<%d and mark_type=1',TABLEPRE,'preward',time()));
// //var_dump($nomark_wk_list);die();
...
// 			}
// 		}//雇主的评价集合，更新
// 		$nomark_gz_list = db_factory::query(sprintf('select `mark_id` from %switkey_mark where model_code="%s" and mark_status=0 and mark_max_time<%d and mark_type=2',TABLEPRE,'preward',time()));
